...
export default function OrderDetailPage() {
  ...
  if (!order) return <div className="empty-state"><h3 className="empty-state__title">Order not found</h3><Link to="/orders" className="btn btn--primary">Back to Orders</Link></div>;

  return (
    <>
      <div className="page-header"><div className="container"><h1 className="page-header__title">Order #{order.id}</h1><div className="page-header__breadcrumb"><Link to="/">Home</Link><BsChevronRight style={{ fontSize: '.7rem' }} /><Link to="/orders">Orders</Link><BsChevronRight style={{ fontSize: '.7rem' }} /><span>#{order.id}</span></div></div></div>
