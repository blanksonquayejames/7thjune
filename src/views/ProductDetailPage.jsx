...
import { BsChevronRight, BsCartPlus, BsBagCheckFill, BsStar, BsStarFill } from 'react-icons/bs';
...
export default function ProductDetailPage() {
  ...
  const discounted = data ? hasActiveDiscount(data) : false;
  const bg = data ? getProductBgColor(data.id) : '';
  const [isAdding, setIsAdding] = useState(false);
  const avgRating = data && data.reviews.length ? (data.reviews.reduce((s, r) => s + r.rating, 0) / data.reviews.length).toFixed(1) : 0;
      ...
      <div className="page-header">
        <div className="container">
          <div className="page-header__breadcrumb">
            ...
            <Link href="/products">Products</Link><BsChevronRight style={{ fontSize: '.7rem' }} />
            <span>{loading ? 'Loading...' : (data?.name || 'Product Not Found')}</span>
          </div>
        </div>
      </div>
                ...
                <div className="pdp-actions">
                  ...
                  >
                    {isAdding ? (
                      ...
                      <><BsBagCheckFill /> In Cart — View Cart</>
                    ) : (
