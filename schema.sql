...
SELECT 
    ...
    CASE 
        WHEN p.discount_percentage > 0 
             AND (p.discount_start IS NULL OR CURRENT_TIMESTAMP >= p.discount_start)
             AND (p.discount_end IS NULL OR CURRENT_TIMESTAMP <= p.discount_end)
...
-- Adjust sequence values to be higher than max IDs to prevent auto-increment conflicts
